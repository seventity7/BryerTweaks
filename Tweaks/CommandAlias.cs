            ...
            if (message->GetCharAt(0) == '/') {
                ...
                if (splitString.Length > 0 && splitString[0].Length >= 2) {
                    ...
                    if (alias != null) {
                        ...
                        if (newStr.Length <= 500) {
                            if (alias.Resend) {
                                if (resendSafety.ElapsedMilliseconds >= 1000) {
                                    ...
                                    Service.Chat.PrintError("[Bryer Tweaks] Something went wrong... You seem to have a command loop");
                                }
                                return;
                            }
                        ...
                        Service.Chat.PrintError("[Bryer Tweaks] " + LocString("CommandTooLongError", "Command alias result is longer than the maximum of 500 characters. The command could not be executed.", "Error: Command is too long"));
                    }
                }
            }
        } catch (Exception ex) {
