#if DEBUG
global using static BryerTweaks.ChangelogConstants;
#endif
...
public class Changelog : Window {
    internal static void AddGeneralChangelogs() {
        ...
        Add("1.9.3.0", "Added optional metrics collection")
            .Sub("Everyone will be given a chance to opt in or our to the collection of a list of enabled tweaks when first accessing the BryerTweaks config window.")
        ...
        Add("1.9.5.0", "The BryerTweaks config window will now be decorated during various festive periods.")
    ...
    private static void Add(ChangelogEntry changelog) {
        ...
        if (HasNewChangelog || BryerTweaks.Plugin.ChangelogWindow.IsOpen) return;
        
        if (Version.TryParse(BryerTweaks.Plugin.PluginConfig.LastSeenChangelog, out var lastVersion)) {
            #if DEBUG
                HasNewChangelog = !BryerTweaks.Plugin.PluginConfig.DisableChangelogNotification && lastVersion < changelog.Version && changelog.Version.Major != 99;
            #else
                HasNewChangelog = !BryerTweaks.Plugin.PluginConfig.DisableChangelogNotification && lastVersion < changelog.Version;    
            #endif

        } else {
            HasNewChangelog = !BryerTweaks.Plugin.PluginConfig.DisableChangelogNotification;
        }
        
        if (HasNewChangelog && BryerTweaks.Plugin.PluginConfig.AutoOpenChangelog) {
    ...
    public Changelog() : base("###simpleTweaksChangelog") {
        ...
        WindowName = $"BryerTweaks Changelog ({CurrentVersion})###simpleTweaksChangelog";
    ...
    public override void OnOpen() {
        ...
        BryerTweaks.Plugin.PluginConfig.LastSeenChangelog = $"{CurrentVersion}";
        BryerTweaks.Plugin.PluginConfig.Save();
                ...
                if (ImGui.TreeNodeEx($"New Tweaks##{version}", ImGuiTreeNodeFlags.DefaultOpen)) {
                    foreach (var c in newTweaks) {
                        ...
                        if (ImGui.IsItemClicked()) {
                            BryerTweaks.Plugin.ConfigWindow.IsOpen = true;
                            BryerTweaks.Plugin.PluginConfig.FocusTweak(c.Tweak);
                        }

                        if (ImGui.IsItemHovered()) {
                            ImGui.SetTooltip($"Click to find '{c.Tweak.Name}' in BryerTweaks config window");
                        }
                ...
                if (ImGui.TreeNodeEx($"Tweak Changes##{version}", ImGuiTreeNodeFlags.DefaultOpen)) {
                    foreach (var group in tweakChanges.OrderBy(g => g.Key?.Name)) {
                        ...
                        if (ImGui.SmallButton($">###{version}_{tweak.Key}_openSettings")) {
                            BryerTweaks.Plugin.ConfigWindow.IsOpen = true;
                            BryerTweaks.Plugin.PluginConfig.FocusTweak(tweak);
                        }
                        if (ImGui.IsItemHovered()) ImGui.SetTooltip($"Find '{tweak.Name}' in BryerTweaks config");
