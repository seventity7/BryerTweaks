...
public class Changelog : Window {
    internal static void AddGeneralChangelogs() {
        ...
        Add("1.9.5.0", "The Bryer Tweaks config window will now be decorated during various festive periods.")
    ...
    public Changelog() : base("###simpleTweaksChangelog") {
        ...
        WindowName = $"Bryer Tweaks Changelog ({CurrentVersion})###simpleTweaksChangelog";
                ...
                if (ImGui.TreeNodeEx($"New Tweaks##{version}", ImGuiTreeNodeFlags.DefaultOpen)) {
                    foreach (var c in newTweaks) {
                        ...
                        if (ImGui.IsItemHovered()) {
                            ImGui.SetTooltip($"Click to find '{c.Tweak.Name}' in Bryer Tweaks config window");
                        }
            ...
            if (tweakChanges.Any()) {
                if (ImGui.TreeNodeEx($"Tweak Changes##{version}", ImGuiTreeNodeFlags.DefaultOpen)) {
                    foreach (var group in tweakChanges.OrderBy(g => g.Key?.Name)) {
                        ...
                        if (ImGui.IsItemHovered()) ImGui.SetTooltip($"Find '{tweak.Name}' in Bryer Tweaks config");
