...
public static unsafe class MetricsService {
    private static BryerTweaks Plugin => BryerTweaks.Plugin;
    private static BryerTweaksConfig Config => Plugin.PluginConfig;
    ...
    public static void ReportMetrics(bool allowFirstUse = false) {
        ...
        if (string.IsNullOrEmpty(identifier) || identifier.Length != 64) {
            ...
            try {
                ...
                BryerTweaks.Plugin.Error(ex, "Error reporting metrics.");
                return;
            }
        }
        ...
        Common.HttpClient.PostAsync("https://metrics.bryertweaks.app/report", new StringContent(json)).ConfigureAwait(false);
    }
